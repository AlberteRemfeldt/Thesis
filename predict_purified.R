...
pred_pur<-function(x,pur_res, interpret=TRUE, cores = 1){
#sapply(1:nrow(x), function(i){
  
  categorical_variables=pur_res[[1]]$categorical_variables
...
if (interpret){
  ...
  pred <- sapply(1:nrow(x),function(i){
    ...
    pred <- unlist(mclapply(1:nrow(x), mc.cores = cores,function(i){
  ...
  return(split(t(pred), colnames(t(pred))))
  # }
  #)
} 
