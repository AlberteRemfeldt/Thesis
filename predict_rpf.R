...
predict_rpf=function(X, forest_res, i=0, cores = 1){
  ...
  if(length(i)==1){
    ...
    if(i==0){
      ...
      if (cores == 1) Y=apply(X,1,F_average, forest_res=forest_res)
      else  Y = unlist(mclapply(1:nrow(X), mc.cores = cores, function(r) F_average(X[r,], forest_res = forest_res)))
  ...
  if (cores == 1) Y=apply(X,1,F_average, forest_res=forest_res, i=i)
  else Y = unlist(mclapply(1:nrow(X), mc.cores = cores, function(r) F_average(X[r,], forest_res=forest_res, i=i)))
